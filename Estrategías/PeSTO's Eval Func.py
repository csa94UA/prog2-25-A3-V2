...
def init_tables():
    for p in range(6):
        ...
        for sq in range(64):
            flip_sq = sq ^ 0x38
            mg_table[white_pc][sq] = mg_value[p] + mg_pesto_table[p][sq]
            eg_table[white_pc][sq] = eg_value[p] + eg_pesto_table[p][sq]
            mg_table[black_pc][sq] = mg_value[p] + mg_pesto_table[p][flip_sq]
            eg_table[black_pc][sq] = eg_value[p] + eg_pesto_table[p][flip_sq]

def evaluate(board, side_to_move):
    mg = [0, 0]
    eg = [0, 0]
    game_phase = 0
    ...
    for sq in range(64):
        pc = board[sq]
        if pc != EMPTY:
            mg[pc % 2] += mg_table[pc][sq]
            eg[pc % 2] += eg_table[pc][sq]
            game_phase += gamephaseInc[pc]

    mg_score = mg[side_to_move] - mg[side_to_move ^ 1]
    eg_score = eg[side_to_move] - eg[side_to_move ^ 1]
